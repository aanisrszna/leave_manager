...
<div class="text-center">
    <div class="d-flex align-items-center justify-content-center">
        <button id="prevMonth" class="btn btn-outline-primary mx-2">&lt;</button>
        <h5 id="monthTitle"><?= date("F", mktime(0, 0, 0, $currentMonth, 1, $currentYear)) ?> <?= $currentYear ?></h5>
        <button id="nextMonth" class="btn btn-outline-primary mx-2">&gt;</button>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateCalendar(month) {
        fetch("update_calendar.php?month=" + month + "&year=" + currentYear)
            .then(response => response.text())
            .then(data => {
                calendarContainer.innerHTML = data;
                monthTitle.innerHTML = new Date(currentYear, month - 1).toLocaleString('default', { month: 'long' }) + " " + currentYear;
            })

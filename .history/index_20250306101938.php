...
if (isset($_POST['signin'])) {
    ...
    // Step 1: Retrieve the stored hashed password and salt from the database
    $stmt = $conn->prepare("SELECT emp_id, role, Department, status, Password, Salt FROM tblemployees WHERE EmailId = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $stored_salt = $row['Salt']; // Salt from DB
        $salted_password = $password . $stored_salt; // Recreate salted password

        // Step 2: Verify using password_verify()
        if (password_verify($salted_password, $stored_hash)) {
				...
				<div class="col-md-6 col-lg-5">
					<div class="login-box bg-white box-shadow border-radius-10">
						<div class="login-title">
							<h2 class="text-center text-primary">Welcome To E-Leave Manager Portal</h2>
						</div>

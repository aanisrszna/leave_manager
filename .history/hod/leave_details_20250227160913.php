...
if (isset($_POST['update'])) {
    ...
    $leave_query = mysqli_query($conn, "
        SELECT l.LeaveType, l.FromDate, l.ToDate, l.Reason, l.RequestedDays, 
               e.EmailId AS staff_email, e.FullName AS staff_name 
        FROM tblleave l
    ...
    $leaveType = $leave_data['LeaveType'];
    $fromDate = $leave_data['FromDate'];
    $toDate = $leave_data['ToDate'];
    $reason = $leave_data['Reason'];
    $requestedDays = $leave_data['RequestedDays'];
    $staffEmail = $leave_data['staff_email'];
    $staffName = $leave_data['staff_name'];
    ...
    if ($status === '1') { // Leave Approved
        $result = mysqli_query($conn, "
            UPDATE tblleave 
            SET 
                HodRemarks = '$status',
                HodSign = '$signature',
                HodDate = '$admremarkdate'
            WHERE id = '$did'
        ");

        if ($result) {
            // Fetch Director Email
            $director_query = mysqli_query($conn, "SELECT EmailId FROM tblemployees WHERE role = 'Director'");
            $director_data = mysqli_fetch_assoc($director_query);
            $directorEmail = $director_data['EmailId'];

            // Email Content for Director
            $subject_director = "Leave Approval Notification - $staffName";
            $message_director = "
                <p>Dear Director,</p>
                <p>The following leave application has been <strong style='color: green;'>approved</strong> by the Head of Department:</p>
                <table border='1' cellpadding='8' cellspacing='0' style='border-collapse: collapse; width: 100%;'>
                    <tr><th align='left'>Employee</th><td>$staffName</td></tr>
                    <tr><th align='left'>Leave Type</th><td>$leaveType</td></tr>
                    <tr><th align='left'>From</th><td>$fromDate</td></tr>
                    <tr><th align='left'>To</th><td>$toDate</td></tr>
                    <tr><th align='left'>Requested Days</th><td>$requestedDays</td></tr>
                    <tr><th align='left'>Reason</th><td>$reason</td></tr>
                </table>
                <p>Please review this request as needed.</p>
                <p>Best regards,<br><strong>e-Leave Manager System</strong></p>
            ";

            send_email($directorEmail, $subject_director, $message_director);
            echo "<script>alert('Leave approved successfully and email sent to Director.');</script>";
        } else {
    ...
    /* 
    // COMMENTED OUT STAFF EMAIL NOTIFICATION
    if ($status === '2') { // Leave Rejected
        ...
        // Email Content for Staff (Leave Rejected)
        $subject_reject = "Leave Application Rejected - $staffName";
        $message_reject = "
            <p>Dear $staffName,</p>
        ...
        // send_email($staffEmail, $subject_reject, $message_reject); // Commented out

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply'])) {
    // Retrieve form data
    $empid = $_SESSION['emp_id']; // Ensure session stores employee ID correctly
    ...
    // Fetch Manager's name and role
    $role_query = "SELECT FirstName, role FROM tblemployees WHERE emp_id = ?";
    $stmt = $conn->prepare($role_query);
    $stmt->bind_param('s', $empid);
    $stmt->execute();
    $role_result = $stmt->get_result();
    $row = $role_result->fetch_assoc();
    $full_name = $row['FirstName']; // Get full name
    $role = $row['role'];
    ...
    if ($overlap_result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            // If the applicant is a Manager, send an email to the Director
            if ($role === 'Manager') {
                // Fetch Director's email
                $director_query = "SELECT EmailId FROM tblemployees WHERE role = 'Director'";
                $director_stmt = $conn->prepare($director_query);
                $director_stmt->execute();
                $director_result = $director_stmt->get_result();

                if ($director_result->num_rows > 0) {
                    while ($director = $director_result->fetch_assoc()) {
                        $director_email = $director['EmailId'];

                        // Email details
                        $subject = "Manager Leave Application Notification";
                        $message = "
                            <html>
                            <head>
                                <title>Manager Leave Application</title>
                                <style>
                            ...
                            <body>
                                <p>Dear Director,</p>
                                <p>The following Manager has applied for leave:</p>
                                <table>
                                    <tr><th>Manager Name</th><td>$full_name</td></tr>
                                    <tr><th>Employee ID</th><td>$empid</td></tr>
                                    <tr><th>Leave Type</th><td>$leave_type</td></tr>
                                    <tr><th>From Date</th><td>$date_from</td></tr>
                                    <tr><th>To Date</th><td>$date_to</td></tr>
                                    <tr><th>Requested Days</th><td>$requested_days</td></tr>
                                    <tr><th>Reason</th><td>$reason</td></tr>
                                </table>
                                <p>Please review and take necessary action.</p>
                                <p>Best regards,<br><b>E-Leave Manager System</b></p>
                            </body>
                            </html>
                        ";
                        ...
                        $headers .= "From: anis@riverraven.com.my" . "\r\n";

                        // Send email
                        send_email($director_email, $subject, $message, $headers);
                    }
                }
            }

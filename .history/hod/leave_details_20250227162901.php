...
if (isset($_POST['update'])) {
    ...
    $query = mysqli_query($conn, "SELECT signature, EmailId FROM tblemployees WHERE emp_id = '$session_id'") or die(mysqli_error($conn));
    $row = mysqli_fetch_assoc($query);
    $signature = $row['signature'];
    $staff_email = $row['EmailId']; // Employee's email
    ...
    if ($status === '2') { // Leave Rejected
        ...
        // Send email to staff about leave rejection
        $subject = "Leave Application Status";
        $message = "<p>Your leave application (ID: $did) has been <b>Rejected</b> by the HOD.</p>";
        ...
        if ($result) {
            ...
            $director_query = mysqli_query($conn, "SELECT EmailId FROM tblemployees WHERE role = 'Director' LIMIT 1");
            $director_row = mysqli_fetch_assoc($director_query);
            $director_email = $director_row['EmailId'];

            // Send email to director about leave approval
            $subject = "Leave Approval Notification";
            $message = "<p>A leave request (ID: $did) has been <b>Approved</b> by the HOD.</p>";

...
if (isset($_POST['update'])) {
    ...
    } elseif ($status === '1') { // Leave Approved (No Deduction)
...
if (isset($_POST['update'])) {
    ...
    $leave_query = mysqli_query($conn, "
        SELECT l.LeaveType, l.FromDate, l.ToDate, l.Reason, e.EmailId AS staff_email, e.FullName AS staff_name
        FROM tblleave l
    ...
    if ($status === '2') { // Leave Rejected
        ...
        send_email($staffEmail, "Leave Application Rejected", $email_body);
        echo "<script>alert('Leave rejected successfully');</script>";
        ...
        if ($result) {
            ...
            $email_body = "
                <p>Dear Director,</p>
                <p>The following leave application has been <strong>approved</strong> by the Head of Department:</p>
                <p><strong>Leave Details:</strong></p>
                <table border='1' cellpadding='8' cellspacing='0' style='border-collapse: collapse; width: 100%;'>
            ...
            send_email($directorEmail, "Leave Approval Notification", $email_body);

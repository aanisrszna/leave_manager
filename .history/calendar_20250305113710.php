...
function draw_calendar($month, $year, $events) {
    ...
    // First Row Adjustment (Handle Overflow)
    $calendar .= "<div class='calendar-row'>";
    $firstRow = [];
    
    if ($dayOfWeek > 0) {
        $firstRow = array_fill(0, $dayOfWeek, "<div class='calendar-cell'></div>");
    }
    ...
    for ($currentDay = 1; $currentDay <= $numberOfDays; $currentDay++) {
        ...
        // Add to first row if it exceeds 5 rows
        if (count($firstRow) < 7) {
            $firstRow[] = "<div class='calendar-cell $eventClass' data-event='$eventName'>$currentDay</div>";
        } else {
            if (($dayOfWeek + 1) % 7 == 0) {

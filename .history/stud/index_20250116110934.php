    ...
    <div class="main-container">
        <div class="pd-ltr-20">
            ...
            if (mysqli_num_rows($latestLeaveQuery) > 0) {
                ...
                // Determine the notification message based on the leave status
                ...
                // Insert a new notification into tblnotifications with is_read = 0 (unread)
                $insertNotificationQuery = mysqli_query($conn, "
                    INSERT INTO tblnotifications (empid, message, is_read) 
                    VALUES ('$emp_id', '$message', 0);
                ") or die(mysqli_error($conn));
            }

            // Check if there are any unread notifications (is_read = 0) for the logged-in employee
            $checkUnreadNotificationsQuery = mysqli_query($conn, "
                SELECT id 
                FROM tblnotifications 
                WHERE empid = '$session_id' AND is_read = 0;
            ") or die(mysqli_error($conn));

            // If there are no unread notifications, display a message
            if (mysqli_num_rows($checkUnreadNotificationsQuery) == 0) {
                echo "<div class='alert alert-warning'>There are no new notifications.</div>";
            } else {
                // Fetch unread notifications and display them
                ...
                while ($notification = mysqli_fetch_array($notificationsQuery)) {
                    ...
                    $updateNotificationQuery = mysqli_query($conn, "

...
include('../includes/email_functions.php'); // ✅ Fix: Correct path to email_function.php

if (isset($_POST['apply'])) {
    $empid = $session_id; // Replace with actual session variable holding the employee ID
    ...
    if (mysqli_num_rows($overlap_result) > 0) {
        ...
        if (mysqli_query($conn, $query)) {
            ...
            $emp_query = "SELECT FirstName, EmailId FROM tblemployees WHERE emp_id = '$empid'";
            ...
            if ($employee) {
                ...
                $leaveDetails = "Leave Type: $leave_type, From: $date_from, To: $date_to, Days: $requested_days";

                // 📌 Send Email Notification
                if (sendNotificationStaff($employee_email, $employee_name, $leave_type, $date_from, $date_to)) {
                    echo "<script>alert('Leave Applied Successfully! Email sent.');</script>";
                } else {
                    echo "<script>alert('Leave Applied Successfully! But email notification failed.');</script>";
                }
            }

            header('Location: leave_history.php');
            exit(); // Ensure the script stops execution immediately
            echo "<script>alert('Leave Applied Successfully!');</script>"; // ❌ Move this after exit() or remove it

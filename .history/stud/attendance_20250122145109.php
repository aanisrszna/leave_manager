                ...
                <tbody id="attendance-records">
                    <?php if (count($attendance_records) > 0): ?>
                        <?php foreach ($attendance_records as $record): ?>
                            <?php
                            // Convert 'time_in' and 'time_out' from UTC to desired timezone (e.g., 'Asia/Kuala_Lumpur')
                            $timezone = new DateTimeZone('Asia/Kuala_Lumpur'); // Change to the desired timezone
                            $time_in = $record['time_in'] ? new DateTime($record['time_in'], new DateTimeZone('UTC')) : null;
                            $time_out = $record['time_out'] ? new DateTime($record['time_out'], new DateTimeZone('UTC')) : null;

                            // Set the timezone for both times
                            if ($time_in) {

                ...
                <tbody id="attendance-records">
                    <?php if (count($attendance_records) > 0): ?>
                        <?php foreach ($attendance_records as $record): ?>
                            <?php
                            // Define timezones
                            $local_timezone = new DateTimeZone('Asia/Kuala_Lumpur'); // Local timezone
                            $utc_timezone = new DateTimeZone('UTC'); // UTC timezone

                            // Check if time is stored in UTC and adjust accordingly
                            $time_in = $record['time_in'] ? new DateTime($record['time_in'], $utc_timezone) : null;
                            $time_out = $record['time_out'] ? new DateTime($record['time_out'], $utc_timezone) : null;

                            // If the time is stored in UTC, we convert it to the local timezone
                            if ($time_in) {
                                $time_in->setTimezone($local_timezone); // Convert to local timezone
                                $time_in_display = $time_in->format('H:i:s'); // Display time in 'HH:mm:ss' format
                            } else {
                                $time_in_display = '-'; // If no time_in, display '-'
                            }

                            if ($time_out) {
                                $time_out->setTimezone($local_timezone); // Convert to local timezone
                                $time_out_display = $time_out->format('H:i:s'); // Display time in 'HH:mm:ss' format
                            } else {
                                $time_out_display = '-'; // If no time_out, display '-'
                            }
                            ?>
                            <tr>

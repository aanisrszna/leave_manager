...
if (isset($_POST['apply'])) {
    $empid = $_SESSION['emp_id']; // Assuming session holds employee ID
    ...
    $needProofQuery = mysqli_query($conn, "SELECT NeedProof FROM tblleavetype WHERE LeaveType = '$leave_type'");
    $needProofRow = mysqli_fetch_assoc($needProofQuery);
    if ($needProofRow['NeedProof'] === 'Yes' && isset($_FILES['proof']['name']) && $_FILES['proof']['error'] == 0) {
        $proofDirectory = '../proof/';
        $proofFileName = time() . '_' . basename($_FILES['proof']['name']);
        $proofFilePath = $proofDirectory . $proofFileName;

        // Validate file size and type
        $allowedTypes = ['image/jpeg', 'image/png', 'application/pdf'];
        if ($_FILES['proof']['size'] > 2 * 1024 * 1024) {
            echo "<script>alert('Proof file is too large. Max size is 2MB.');</script>";
            exit();
        }
        if (!in_array($_FILES['proof']['type'], $allowedTypes)) {
            echo "<script>alert('Invalid proof file type. Only JPG, PNG, and PDF are allowed.');</script>";
            exit();
        }
    ...
    $query = "INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, proof)
              VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$proofFilePath')";
                    ...
                    <div class="wizard-content">
                        <form action="apply_leave.php" method="post" enctype="multipart/form-data">
                            <section>
                                ...
                                <div class="form-group" id="proof_container" style="display: none;">
                                    <label for="proof">Upload Proof (if required)</label>
...
<script>
    ...
    document.getElementById('leave_type').addEventListener('change', toggleProofField);

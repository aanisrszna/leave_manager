...
if (isset($_POST['apply'])) {
    ...
    // Assuming you are getting the signature as a file (image)
    if (isset($_FILES['signature']) && $_FILES['signature']['error'] == 0) {
        ...
        // Move the uploaded signature to the server directory
        move_uploaded_file($signature_tmp_name, $signature_path);
    } else {
        $signature_path = ''; // If no signature is uploaded, set it to an empty string
    }
    ...
    if (mysqli_num_rows($overlap_result) > 0) {
        ...
        $query = "
            INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, Sign)
            VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$signature_path')
        ";

...
if (isset($_POST['apply'])) {
    ...
    if ($proof_file['error'] === UPLOAD_ERR_OK) {
        $proof_file_name = time() . '_' . basename($proof_file['name']);
        $upload_dir = '../uploads/';
        $target_path = $upload_dir . $proof_file_name;
        ...
        echo "Error during file upload: " . $proof_file['error'];
    }
    ...
    if (mysqli_num_rows($overlap_result) > 0) {
        ...
        $query = "
            ...
            VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$proof_file_name')
        ";

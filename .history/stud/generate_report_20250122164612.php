...
if (isset($_POST['generate_report'])) {
    ...
    $pdf->Cell(0, 10, "$monthName 2025 - Events & Analysis", 0, 1, 'L');
    $pdf->Ln(5);

    // List holidays, birthdays, and leaves
    $pdf->SetFont('helvetica', '', 12);

    // Holidays
    $pdf->Cell(0, 10, 'Public Holidays:', 0, 1, 'L');
    foreach ($malaysiaHolidays as $date => $event) {
        $pdf->Cell(0, 10, "$date - $event", 0, 1, 'L');
    }
    $pdf->Ln(5);

    // Birthdays
    $pdf->Cell(0, 10, 'Employee Birthdays:', 0, 1, 'L');
    ...
    // Leave Events
    $pdf->Cell(0, 10, 'Employee Leave Events:', 0, 1, 'L');

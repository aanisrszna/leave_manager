                ...
                <tbody id="attendance-records">
                    <?php if (count($attendance_records) > 0): ?>
                        <?php foreach ($attendance_records as $record): ?>
                            <?php
                            // Ensure time is in UTC before converting to local time zone
                            $timezone = new DateTimeZone('Asia/Kuala_Lumpur'); // Local timezone (change as needed)
                            $utc_timezone = new DateTimeZone('UTC'); // UTC timezone
                            
                            // Convert time_in and time_out to UTC first (if stored in UTC), then change to local time zone
                            ...
                            // Set the local timezone for both times
                            if ($time_in) {
                                $time_in->setTimezone($timezone);
                                $time_in_display = $time_in->format('H:i:s');
                            } else {
                                $time_in_display = '-';
                            }

                            if ($time_out) {
                                $time_out->setTimezone($timezone);
                                $time_out_display = $time_out->format('H:i:s');
                            } else {
                                $time_out_display = '-';
                            }
                            ?>

...
<script>
    function updateProofRequirement() {
        const leaveTypeSelect = document.getElementById('leave_type');
        const selectedOption = leaveTypeSelect.options[leaveTypeSelect.selectedIndex];
        const needProof = selectedOption.getAttribute('data-need-proof') || 'No';

        const proofContainer = document.getElementById('proof-container');
        
        // Show or hide the proof upload based on NeedProof value
        if (needProof === 'Yes') {
            proofContainer.style.display = 'block'; // Show file upload
        } else {
            proofContainer.style.display = 'none'; // Hide file upload
        }
    }

    // Trigger proof check when the leave type changes
    document.getElementById('leave_type').addEventListener('change', updateProofRequirement);
</script>

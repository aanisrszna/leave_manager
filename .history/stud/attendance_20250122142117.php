...
<body>
    ...
    $date = date('Y-m-d');
    $result = $conn->query("SELECT * FROM tblattendance WHERE date = '$date' AND staff_id = '{$_SESSION['alogin']}'");
    $attendance_data = $result->fetch_assoc();
    $clocked_in = isset($attendance_data['time_in']) && !$attendance_data['time_out'];
    ?>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="attendance-card">
                    ...
                    <div class="tabs">
                        <button class="tab-button <?= $clocked_in ? '' : 'active'; ?>" onclick="switchTab('clock-in')">Clock In</button>
                        <button class="tab-button <?= $clocked_in ? 'active' : ''; ?>" onclick="switchTab('clock-out')">Clock Out</button>
                    </div>
                    <form method="POST" id="clock-in-form" style="<?= $clocked_in ? 'display: none;' : ''; ?>">
                        <input type="hidden" name="staff_id" value="<?= $_SESSION['alogin']; ?>">
                    ...
                    <form method="POST" id="clock-out-form" style="<?= !$clocked_in ? 'display: none;' : ''; ?>">
                        <input type="hidden" name="staff_id" value="<?= $_SESSION['alogin']; ?>">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php if ($attendance_data): ?>
                            <tr>
                                <td><?= date('M d, Y', strtotime($attendance_data['date'])); ?></td>
                                <td><?= $attendance_data['time_in'] ?: '-'; ?></td>
                                <td><?= $attendance_data['time_out'] ?: '-'; ?></td>
                                <td>
                                    <?php
                                    if ($attendance_data['total_hours']) {
                                        $hours = floor($attendance_data['total_hours'] / 3600);
                                        $minutes = floor(($attendance_data['total_hours'] % 3600) / 60);
                                        $seconds = $attendance_data['total_hours'] % 60;
                                ...
                                <td>
                                    <?= $attendance_data['time_out'] ? 'Out' : 'In'; ?>
                                </td>
                            </tr>
                        <?php else: ?>
                            <tr>
    ...
    <script>
        ...
        function updateTime() {
            const now = new Date();
            document.getElementById('time').textContent = now.toLocaleTimeString('en-GB'); // 24-hour format
        }

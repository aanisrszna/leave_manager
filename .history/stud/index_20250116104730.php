...
<body>
    ...
    <div class="main-container">
        <div class="pd-ltr-20">
            ...
            <?php
            // Fetch the most recent leave ID for the logged-in employee
            ...
            if (mysqli_num_rows($latestLeaveQuery) > 0) {
                ...
                // Now update the empid based on the RegRemarks condition
                if ($leave['RegRemarks'] == 1) {
                    // If leave is approved, update the empid in tblleave (you can change the action as per your needs)
                    $updateQuery = mysqli_query($conn, "
                        UPDATE tblleave 
                        SET empid = 'new_empid'  -- Replace 'new_empid' with the actual empid to update
                        WHERE id = '$leave_id';
                    ") or die(mysqli_error($conn));
                    
                    echo "<div class='alert alert-success'>🎉 Your Leave Has Been Approved! EmpID updated.</div>";
                } elseif ($leave['RegRemarks'] == 2) {
                    // If leave is rejected, you can perform other actions here if necessary

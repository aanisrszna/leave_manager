...
if (isset($_POST["apply"])) {
    ...
    // Initialize proof variable
    $proof_file = NULL;

    // Check if the leave type requires proof
    $query_leave_type = mysqli_query($conn, "
        SELECT NeedProof FROM tblleavetype WHERE LeaveType = '$leave_type'
    ");
    $row_leave_type = mysqli_fetch_assoc($query_leave_type);

    // If proof is required and a file is uploaded
    if ($row_leave_type['NeedProof'] == 'Yes' && isset($_FILES['proof']) && $_FILES['proof']['error'] == 0) {
        ...
        $proof_name = uniqid('proof_') . '_' . basename($_FILES['proof']['name']);
        $proof_file = $upload_dir . $proof_name;

        // Move the uploaded file to the desired directory
        if (!move_uploaded_file($_FILES['proof']['tmp_name'], $proof_file)) {
            echo "<script>alert('Error uploading proof.');</script>";
            exit();
        }
    }

    // Insert leave application into the database
    $query = "INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, proof)
              VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$proof_file')";
                    ...
                    <div class="wizard-content">
                        <form method="post" action="" enctype="multipart/form-data">
                            <section>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="leave_type" id="leave_type" class="custom-select form-control" required="true" onchange="updateAvailableDays()">
                                                ...
                                                while ($row = mysqli_fetch_assoc($query)) {
                                                    echo '<option value="' . $row['LeaveType'] . '" data-available-days="' . $row['available_day'] . '" data-need-proof="' . $row['NeedProof'] . '">'
                                ...
                                <div class="row" id="upload_proof_container" style="display: none;">
                                    <div class="col-md-12 col-sm-12">
                                        <div class="form-group">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
            // Show/Hide Upload Proof container based on leave type
            document.getElementById('leave_type').addEventListener('change', function () {
                var needProof = this.options[this.selectedIndex].getAttribute('data-need-proof');
                var uploadProofContainer = document.getElementById('upload_proof_container');
                var proofInput = document.querySelector('input[name="proof_file"]');
            ...
            document.querySelector('input[name="proof_file"]').addEventListener('change', function () {
            ...
            document.querySelector('form').addEventListener('submit', function (e) {
                ...
                var needProof = leaveType.options[leaveType.selectedIndex].getAttribute('data-need-proof');
                var proofInput = document.querySelector('input[name="proof_file"]');

                if (needProof === 'Yes' && !proofInput.value) {
                    alert('Please upload proof for this leave type.');

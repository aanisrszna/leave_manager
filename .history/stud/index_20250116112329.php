...
<body>
    ...
    <div class="main-container">
        <div class="pd-ltr-20">
            ...
            <?php
            // Start session to use session variables, only if session is not already active
            if (session_status() == PHP_SESSION_NONE) {
                session_start();
            }

            // Check if the message has already been displayed
            if (!isset($_SESSION['leave_message_shown'])) {
                ...
                if (mysqli_num_rows($latestLeaveQuery) > 0) {
                    ...
                    if ($leave['RegRemarks'] == 1) {
                        ...
                        echo "<div class='alert alert-success' id='leaveAlert'>🎉 Your Leave Has Been Approved!
                                <button class='close' onclick='closeAlert()'>&times;</button>
                            </div>";
                    } elseif ($leave['RegRemarks'] == 2) {
                        // Leave rejected notification
                        echo "<div class='alert alert-danger' id='leaveAlert'>😔 Sorry, Your Leave Has Been Rejected.
                                <button class='close' onclick='closeAlert()'>&times;</button>
                            </div>";
                    }
                } else {
                    echo "<div class='alert alert-warning' id='leaveAlert'>No leave record found for this employee.
                            <button class='close' onclick='closeAlert()'>&times;</button>
                        </div>";
                }

                // Set session variable to indicate the message has been shown
                $_SESSION['leave_message_shown'] = true;
            }
            ?>

            <script>
                // Function to close the alert when the button is clicked
                function closeAlert() {
                    var alertMessage = document.getElementById('leaveAlert');
                    if (alertMessage) {

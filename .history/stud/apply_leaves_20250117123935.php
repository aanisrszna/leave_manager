...
if (isset($_POST['apply'])) {
    $empid = $session_id; // Employee ID from session
    ...
    if (isset($_FILES['proof_file']) && $_FILES['proof_file']['error'] === UPLOAD_ERR_OK) {
        $uploadDir = '../uploads/proof_files/';
        $proofFileName = basename($_FILES['proof_file']['name']);
        $proofFileTmp = $_FILES['proof_file']['tmp_name'];
        $proofFilePath = $uploadDir . uniqid() . '_' . $proofFileName;
        ...
        if (!move_uploaded_file($proofFileTmp, $proofFilePath)) {
            echo "<script>alert('Error uploading proof file.');</script>";
            $proofFilePath = null; // Reset proof file path if upload fails
        }
    }
    ...
    if (mysqli_num_rows($overlap_result) > 0) {
        ...
        // Insert leave application with proof file
        $query = "
            INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, ProofFilePath)
                                ...
                                <div class="row">
                                    <div class="col-md-12 col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="file" name="proof_file" id="proof_file" class="form-control" accept=".pdf,.jpg,.jpeg,.png">
                                        </div>
                                    </div>
                                </div>                                

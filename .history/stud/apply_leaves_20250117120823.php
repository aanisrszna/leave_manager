...
if (isset($_POST['apply'])) {
    ...
    if ($proof_error === 0) {
        ...
        $file_ext = pathinfo($proof_name, PATHINFO_EXTENSION);

        if (in_array(strtolower($file_ext), $allowed_extensions)) {
            if ($proof_size <= 5 * 1024 * 1024) { // 5MB max file size
                $new_proof_name = uniqid('proof_', true) . '.' . $file_ext;
                $proof_destination = $upload_dir . $new_proof_name;

                // Move the uploaded file
                if (move_uploaded_file($proof_tmp_name, $proof_destination)) {
                    // Insert leave application into database
                    $sql = "INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, Description, Status, proof) 
                            VALUES (:empid, :leavetype, :fromdate, :todate, :description, :status, :proof)";
                    ...
                    $query->bindParam(':proof', $new_proof_name, PDO::PARAM_STR);
                    $query->execute();

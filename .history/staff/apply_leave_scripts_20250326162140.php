...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const proofMessage = document.createElement('span'); // Create a span for the message
    proofMessage.style.color = "green"; // Set success message color
    proofMessage.style.marginLeft = "10px"; // Adjust spacing
    proofInput.parentNode.appendChild(proofMessage); // Append message to proof input field

    proofInput.addEventListener('change', function () {
        if (proofInput.files.length > 0) {
            proofMessage.textContent = "Proof uploaded successfully ✅";
        } else {
            proofMessage.textContent = ""; // Clear message if no file
        }
        validateForm();
    });
});

...
function calc() {
    ...
    const leaveTypeSelect = document.getElementById('leave_type'); // Get leave type dropdown
    const leaveTypeId = parseInt(leaveTypeSelect.value); // Get selected leave type ID as integer
    ...
    if (leaveTypeId !== 4 && startDate < minBusinessDate) {
        alert("The leave application must be submitted at least five business days before the start date.");
        resetInputs();
        return;
    }

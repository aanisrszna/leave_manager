...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        var existingChart = Chart.getChart("approvedLeaveBarChart"); // Check if chart exists

        if (existingChart) {
            // Update existing chart
            existingChart.data.labels = employeeNames;
            existingChart.data.datasets[0].data = leaveCountsAnnual;
            existingChart.data.datasets[1].data = leaveCountsMedical;
            existingChart.update(); // Update chart without reloading
        } else {
            // Create new chart
            new Chart(ctx, {
                type: 'bar',

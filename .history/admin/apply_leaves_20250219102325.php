...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply'])) {
    ...
    if (!empty($_FILES['proof']['name']) && $_FILES['proof']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes) && $_FILES['proof']['size'] <= 2 * 1024 * 1024) {
            if (move_uploaded_file($_FILES['proof']['tmp_name'], $targetFilePath)) {
                ...
                $_SESSION['error_message'] = 'Failed to upload proof file.';
                header('Location: apply_leave.php');
                exit();
            }
        } else {
            $_SESSION['error_message'] = 'Invalid file type or file size exceeds 2MB.';
            header('Location: apply_leave.php');
            exit();
        }
    }
    ...
    if ($overlap_result->num_rows > 0) {
        $_SESSION['error_message'] = 'You already applied for this date. Delete it first before submitting a new application.';
        header('Location: apply_leaves.php');
        exit();
    }

    // Fetch employee role
    $role_query = "SELECT role, FirstName FROM tblemployees WHERE emp_id = ?";
    ...
    if (!$employee) {
        $_SESSION['error_message'] = 'Error fetching employee details.';
        header('Location: apply_leave.php');
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['error_message'] = 'Error applying leave: ' . $stmt->error;
        header('Location: apply_leave.php');
        exit();
    }
}

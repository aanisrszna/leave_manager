...
<script>
    function calc() {
        ...
        if (isWeekend(startDate) || isWeekend(endDate)) {
            alert("Leave cannot be requested on weekends (Saturday or Sunday)!");
            resetInputs();
            return;
        }

        // Check if applying at least 5 days in advance
        const diffInDays = Math.ceil((startDate - today) / (1000 * 60 * 60 * 24));
        if (diffInDays < 5) {
            alert("Leave must be applied at least 5 days before the start date!");
            resetInputs();
            return;
        }

        // Calculate requested leave days (business days only)
        const requestedDays = getBusinessDateCount(startDate, endDate);
        document.getElementById('requested_days').value = requestedDays;

        // Update Outstanding Days
        updateAvailableDays();
    }
    ...
    function updateAvailableDays() {
        ...
        const requestedDays = document.getElementById('requested_days').value || 0;
        ...
        // Optional: Alert if requested days exceed available days
    ...
    function getBusinessDateCount(startDate, endDate) {
        ...
        elapsed = (elapsed / 7) * 5;
        elapsed += (daysBeforeFirstSunday > 1 ? daysBeforeFirstSunday - 1 : 0) + (daysAfterLastSunday < 6 ? daysAfterLastSunday : 5);

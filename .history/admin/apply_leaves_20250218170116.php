...
if (isset($_POST['apply'])) {
    ...
    $stmt->bind_param('sssssss', $empid, $date_from, $date_to, $date_from, $date_to, $date_from, $date_to, $reason);
    $stmt->execute();
    ...
    if ($overlap_result->num_rows > 0) {
        ...
        $insert_query = "
            INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, Proof, reason)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param('ssssssss', $empid, $leave_type, $date_from, $date_to, $requested_days, $outstanding_days, $datePosting, $proof, $reason);

        if ($stmt->execute()) {
            // Fetch Employee Details for Email
            $emp_query = "SELECT FirstName, EmailId FROM tblemployees WHERE emp_id = ?";
            ...
            if ($employee) {
                ...
                if (sendNotificationStaff($employee_email, $employee_name, $leave_type, $date_from, $date_to)) {
                    $success_message = 'Leave Applied Successfully! Email sent.';
                } else {
                    $error_message = 'Leave Applied Successfully! But email notification failed.';
                }
            }
    ...
    $role_query = "SELECT role FROM tblemployees WHERE emp_id = '$empid'";
    ...
    $query = "INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, HodRemarks, reason) 
              VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$hod_remarks', '$resaon')";
...
<?php
    if (isset($_POST['apply'])) {
        ...
        $hod_remarks = ($role == 'Manager') ? 3 : 'Pending';
        // Insert leave application into the database
        $query = "INSERT INTO tblleave (empid, LeaveType, FromDate, ToDate, RequestedDays, DaysOutstand, PostingDate, HodRemarks) 
              VALUES ('$empid', '$leave_type', '$date_from', '$date_to', '$requested_days', '$outstanding_days', '$datePosting', '$hod_remarks')";

...
if (isset($_POST['add_staff'])) {
    ...
    if ($count > 0) {
        ...
        // Generate a salt (optional - PASSWORD_BCRYPT already includes a salt)
        $salt = bin2hex(random_bytes(16)); // Generate a random 16-byte salt
        $salted_password = $password . $salt; // Append salt to password
        $hashed_password = password_hash($salted_password, PASSWORD_BCRYPT); // Hash the salted password

        // Insert the new staff record
        $sql = "INSERT INTO tblemployees (
            FirstName, EmailId, Password, Salt, Gender, Dob, Department, Address, role, Phonenumber, Status, location, Staff_ID, Position_Staff, Date_Joined, Emergency_Contact, Car_Plate, Reporting_To, IC_Number, Service_Year, Emergency_Name, Emergency_Relation
        ) VALUES (
            '$fname', '$email', '$hashed_password', '$salt', '$gender', '$dob', '$department', '$address', '$user_role', '$phonenumber', '$status', 'NO-IMAGE-AVAILABLE.jpg', '$staff_id', '$position_staff', '$date_joined', '$emergency_contact', '$car_plate', '$reporting_to', '$ic_number', '$service_year', '$emergency_name', '$emergency_relation'
        )";
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input name="dob" id="dob" type="text" class="form-control date-picker" required readonly="<?php echo date('d F Y', strtotime($dob)); ?>">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">

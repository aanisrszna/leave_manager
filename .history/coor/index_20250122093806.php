				...
				foreach ([$malaysiaHolidays, $birthdays, $leaveDates] as $eventArray) {
					foreach ($eventArray as $date => $event) {
						if (!isset($calendarEvents[$date])) {
							...
							// Apply priority: holiday > birthday > leave
							if ($event[1] === 'holiday') {
								$calendarEvents[$date] = $event;
							} elseif ($event[1] === 'birthday' && $calendarEvents[$date][1] !== 'holiday') {
								$calendarEvents[$date] = $event;
							} elseif ($event[1] === 'leave' && $calendarEvents[$date][1] === null) {
								$calendarEvents[$date] = $event;
							}
						}
					}
				...
				function draw_calendar($month, $year, $events) {
					...
					while ($currentDay <= $numberOfDays) {
						...
						if ($eventData) {
							...
							$eventType = $eventData[1] ?? ''; // Event type
				
							if ($eventType === 'holiday') {
								$backgroundClass = 'bg-danger text-white'; // Red for holidays
							} elseif ($eventType === 'birthday') {
								$backgroundClass = 'bg-warning text-dark'; // Yellow for birthdays
							} elseif ($eventType === 'leave') {
								$backgroundClass = 'bg-primary text-white'; // Blue for leave
							}
						}

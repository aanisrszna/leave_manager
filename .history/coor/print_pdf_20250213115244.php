...
$sql = "SELECT tblemployees.FirstName, tblemployees.LastName, tblemployees.Staff_ID, tblemployees.Position_Staff, tblemployees.Phonenumber, tblemployees.EmailId, 
               tblleave.LeaveType, tblleave.RequestedDays, tblleave.DaysOutstand, tblleave.PostingDate, tblleave.FromDate, tblleave.ToDate, tblleave.Reason, 
...
$hod_sign = $row['HodSign'];  // HOD Signature
$reg_sign = $row['RegSign'];  // Director's Signature
...
<table cellspacing="0" cellpadding="5" border="1">
    ...
    <tr><td><b>Reason:</b></td><td>$leave_reason</td></tr>
</table>
<br>
<h3>Approvals</h3>
<table cellspacing="0" cellpadding="5" border="1">
...
if (!empty($hod_sign) && file_exists($hod_sign)) {
    $pdf->Image($hod_sign, 15, $pdf->GetY() + 10, 50, 20, '', '', 'T', true, 300, '', false, false, 1, false, false, false);
    $pdf->Write(0, 'HOD Signature', '', 0, 'L', true, 0, false, false, 0);
}
if (!empty($reg_sign) && file_exists($reg_sign)) {
    $pdf->Image($reg_sign, 100, $pdf->GetY(), 50, 20, '', '', 'T', true, 300, '', false, false, 1, false, false, false);
    $pdf->Write(0, 'Director Signature', '', 0, 'L', true, 0, false, false, 0);
}

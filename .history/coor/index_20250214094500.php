			...
			<div class="row pb-10">
				...
				<div class="col-xl-3 col-lg-3 col-md-6 mb-20">
					<div class="card-box height-100-p widget-style3">
						...
						<?php
						$status = 0;
						$excludedRole = ['manager', 'admin']; // Assuming the role for manager is stored as 'manager' in the tblemployees table
            ...
            <div class="calendar-container">
				...
				foreach ([$malaysiaHolidays, $birthdays, $leaveDates] as $eventArray) {
					foreach ($eventArray as $date => $event) {
						...
						$calendarEvents[$date][] = $event; // Allow multiple events per date
					}
				}
				...
				function draw_calendar($month, $year, $events) {
					...
					while ($currentDay <= $numberOfDays) {
						...
						if ($eventData) {
							// Determine background color by priority: holiday > birthday > leave
							foreach ($eventData as $event) {
								if ($event[1] === 'holiday') {
									$backgroundClass = 'bg-danger text-white'; // Holiday background
								} elseif ($event[1] === 'birthday' && !$backgroundClass) {
									$backgroundClass = 'bg-warning text-dark'; // Birthday background
								} elseif ($event[1] === 'leave' && !$backgroundClass) {
									$backgroundClass = 'bg-primary text-white'; // Leave background
								}
				
								// Add all event names
								$eventNames .= "<small>{$event[0]}</small><br>";
							}
						}
				
						$calendar .= "<td class='text-center $backgroundClass'>";
						$calendar .= $currentDay;
				
						if ($eventNames) {
							$calendar .= "<br>$eventNames";
						}

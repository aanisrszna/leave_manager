            ...
            <div class="calendar-container">
				...
				function draw_calendar($month, $year, $events) {
					...
					while ($currentDay <= $numberOfDays) {
						...
						if ($eventData) {
							// Handle multiple events (e.g., holiday + birthday)
							if (count($eventData) > 1) {
								// Concatenate event names only, ignore event types
								$eventName = implode('<br>', array_column($eventData, 0)); // Get event names
							} else {
								$eventName = $eventData[0]; // Only event name
							}
						}

						$eventClass = '';
						if (isset($eventData[1])) {
							$eventType = $eventData[1]; // Get event type
						...
						$calendar .= $currentDay;
						if ($eventName) {
							$calendar .= "<br><small>$eventName</small>"; // Display only event name
						}
						$calendar .= "</td>";

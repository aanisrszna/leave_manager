            ...
            <div class="calendar-container">
				...
				function draw_calendar($month, $year, $events) {
					...
					while ($currentDay <= $numberOfDays) {
						...
						$eventName = $eventData[0] ?? ''; // Only get the event name (no type)

						$eventClass = '';
						if (isset($eventData[1])) {
							$eventType = $eventData[1]; // Get event type for styling
						...
						$calendar .= $currentDay;
						if ($eventName) {
							$calendar .= "<br><small>$eventName</small>"; // Display only the event name
						}
						$calendar .= "</td>";

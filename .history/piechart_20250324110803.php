...
$sqlAnnual = "SELECT e.FirstName, COUNT(l.id) AS leave_count 
        ...
        JOIN tblleavetype lt ON l.LeaveType = lt.id
        WHERE l.RegRemarks = :status 
        AND YEAR(l.FromDate) = :currentYear
        AND lt.id IN (1, 2, 3)  
...
// Extract employee names and leave counts separately
$employeeNamesAnnual = [];
$leaveCountsAnnual = [];
foreach ($resultsAnnual as $row) {
    $employeeNamesAnnual[] = $row['FirstName'];
    $leaveCountsAnnual[] = $row['leave_count'];
}

$employeeNamesMedical = [];
$leaveCountsMedical = [];
foreach ($resultsMedical as $row) {
    $employeeNamesMedical[] = $row['FirstName'];

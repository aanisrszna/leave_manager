...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        var employeeNames = <?php echo json_encode(array_keys($leaveCountsAnnual)); ?>;
        var leaveCountsAnnual = <?php echo json_encode(array_map('floatval', array_values($leaveCountsAnnual))); ?>;
        var leaveCountsMedical = <?php echo json_encode(array_map('floatval', array_values($leaveCountsMedical))); ?>;

        console.log("Employee Names:", employeeNames);
        console.log("Annual Leave Counts:", leaveCountsAnnual);
        console.log("Medical Leave Counts:", leaveCountsMedical);
        ...
        // Destroy existing chart if it exists (Fixes 'Canvas is already in use' error)
        if (window.myChart) {
                    ...
                    y: {
                        ...
                        ticks: {
                            ...
                            callback: function (value) {
                                return value.toFixed(2); // Display as '1.00' instead of '1'
                            }
                        }
                    }
                },
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (tooltipItem) {
                                ...
                                let value = tooltipItem.raw || 0;
                                let category = datasetIndex === 0 ? "Annual Leave" : "Medical Leave";
                                return `${value.toFixed(2)} days (${category})`;
                            }
                        }
                    }
                }

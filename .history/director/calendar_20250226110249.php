...
while ($row = mysqli_fetch_array($leaveQuery)) {
    if ($row['RegRemarks'] == 1) {
        ...
        while ($fromDate <= $toDate) {
            ...
            $leaveDates[$formattedDate] = [$row['FirstName'] . " on Leave 🌊", 'leave'];
...
function draw_calendar($month, $year, $events) {
    ...
    while ($currentDay <= $numberOfDays) {
        ...
        $calendar .= "<div class='calendar-cell $eventClass' onmouseover='showTooltip(event, \"$eventName\")' onmouseout='hideTooltip()'>";
        $calendar .= $currentDay;
        $calendar .= "</div>";

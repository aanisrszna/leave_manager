...
// Function to generate the calendar with fixed cell sizes
function draw_calendar($month, $year, $events) {
    ...
    if ($dayOfWeek > 0) {
        $calendar .= str_repeat("<td class='empty-cell'></td>", $dayOfWeek);
    }
    ...
    while ($currentDay <= $numberOfDays) {
        ...
        $calendar .= "<td class='text-center fixed-cell $eventClass' data-bs-toggle='tooltip' title='$eventTooltip'>";
        $calendar .= "<div>$currentDay</div>";
        if (!empty($eventLabel)) {
            $calendar .= "<div class='event-tooltip'>$eventLabel</div>";
        }
        $calendar .= "</td>";
    ...
    if ($dayOfWeek != 7) {
        $calendar .= str_repeat("<td class='empty-cell'></td>", 7 - $dayOfWeek);
    }
...
<form method="GET">
    <select name="month"><?php foreach ($months as $num => $name) {

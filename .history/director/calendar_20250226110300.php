...
function draw_calendar($month, $year, $events) {
    ...
    while ($currentDay <= $numberOfDays) {
        ...
        $tooltip = $eventName ? "title='$eventName'" : "";

        $calendar .= "<div class='calendar-cell $eventClass' $tooltip>";
        $calendar .= $currentDay;
        $calendar .= "</div>";
...
<!-- Month Selection Form (Smaller & Centered) -->
<div class="calendar-header">
    <form method="GET">
        <button type="submit" name="month" value="<?= $previousMonth ?>" class="nav-arrow">&lt;</button>
        <span class="month-label"><?= date('F', mktime(0, 0, 0, $currentMonth, 1, $currentYear)) ?> <?= $currentYear ?></span>
        <button type="submit" name="month" value="<?= $nextMonth ?>" class="nav-arrow">&gt;</button>
    </form>
</div>

...
function draw_calendar($month, $year, $events) {
    ...
    // If dates exceed 35, move excess dates to the first column of the first row
    $excessDaysStart = $startIndex + $numberOfDays; // First out-of-bounds index
    if ($excessDaysStart > 35) {
        $extraDays = $excessDaysStart - 35;
        for ($i = 0; $i < $extraDays; $i++) {
            $calendarDays[$i * 7] = 35 + $i - $startIndex + 1; // Move to first column
        }
        // Remove duplicated extra days from bottom
        array_splice($calendarDays, 35, $extraDays);
    }

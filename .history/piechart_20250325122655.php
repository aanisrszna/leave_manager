...
$sql = "SELECT e.emp_id, e.LastName, lt.id AS leave_type, SUM(l.RequestedDays) AS leave_count
        FROM tblleave l 
...
foreach ($results as $row) {
    $empId = $row['emp_id']; // Use emp_id to ensure unique employee entries
    $name = $row['LastName'];
    $leaveType = intval($row['leave_type']);
    $leaveDays = floatval($row['leave_count']);

    // Store employee names with emp_id as key
...
foreach ($employees as $empId => $name) {
    $leaveCountsAnnual[$empId] = $leaveCountsAnnual[$empId] ?? 0;
    $leaveCountsMedical[$empId] = $leaveCountsMedical[$empId] ?? 0;
}

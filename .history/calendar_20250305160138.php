...
foreach ([$malaysiaHolidays, $birthdays, $leaveDates] as $eventTypeArray) {
    foreach ($eventTypeArray as $date => $eventData) {
        if (isset($calendarEvents[$date])) {
            $calendarEvents[$date][0] .= "<br>" . $eventData[0]; // Append event names
            $calendarEvents[$date][1] = 'multiple'; // Mark as multiple events
        } else {
            $calendarEvents[$date] = $eventData;
        }
    }
}
...
function draw_calendar($month, $year, $events) {
    ...
    $totalCells = 35; // 5 rows × 7 columns
    ...
    // Handle extra days by wrapping them back to the first row
    $excessDaysStart = $startIndex + $numberOfDays; // First out-of-bounds index
    if ($excessDaysStart > 35) {
        $extraDays = $excessDaysStart - 35;
        for ($i = 0; $i < $extraDays; $i++) {
            $calendarDays[$i] = 35 + $i - $startIndex + 1; // Place in first row
        }
        array_splice($calendarDays, 35); // Trim array to exactly 35 cells
    }
